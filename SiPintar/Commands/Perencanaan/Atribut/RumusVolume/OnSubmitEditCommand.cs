...
namespace SiPintar.Commands.Perencanaan.Atribut.RumusVolume
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(RumusVolumeViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                if (property.Name != "IdPdam" &&
                    property.Name != "IdUserRequest" &&
                    property.Name != "IdOngkos" &&
                    property.Name != "FlagHapus" &&
                    property.GetValue(_viewModel.FormData) != null)
                {
            ...
            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-rumus-volume-ongkos", null, body));
            if (!Response.IsError)
            {
