...
namespace SiPintar.Commands.Perencanaan.Atribut.Ongkos
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(OngkosViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                {
                    body.Add(property.Name, property.GetValue(_viewModel.OngkosForm));
                }
            }

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-ongkos", null, body));
            if (!Response.IsError)
            {
