...
namespace SiPintar.Commands.Gudang.MasterData.Barang
{
    public class OnSubmitEditFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditFormCommand(BarangViewModel _vm, IRestApiClientModel _restApi, bool _isTest = false)
        {
            Vm = _vm;
            RestApi = _restApi;
            IsTest = _isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            Param.Add("idBarang", _param.IdBarang);
            ...
            var Response = await Task.Run(() => RestApi.PatchAsync($"/api/{ApiVersion}/master-barang", null, Param));
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status)
                {
                    await UploadFotoAsync(_param.IdBarang);
