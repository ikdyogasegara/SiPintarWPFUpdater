...
namespace SiPintar.Commands.Gudang.MasterData.KategoriBarangKeluar
{
    public class OnSubmitEditFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditFormCommand(KategoriBarangKeluarViewModel _vm, IRestApiClientModel _restApi, bool _isTest = false)
        {
            Vm = _vm;
            RestApi = _restApi;
            IsTest = _isTest;
        ...
        {
            var _param = parameter as ParamMasterKategoriBarangKeluarDto;
            ...
            Param.Add("idKategoriBarangKeluar", _param.IdKategoriBarangKeluar);
            Param.Add("kategori", _param.Kategori);
            Param.Add("kodeNomor", _param.KodeNomor);
            var Response = await Task.Run(() => RestApi.PatchAsync($"/api/{ApiVersion}/master-kategori-barang-keluar", null, Param));
