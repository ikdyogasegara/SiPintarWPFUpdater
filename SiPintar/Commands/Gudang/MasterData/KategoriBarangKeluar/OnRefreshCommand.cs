...
namespace SiPintar.Commands.Gudang.MasterData.KategoriBarangKeluar
{
    public class OnRefreshCommand : AsyncCommandBase
    {
        ...
        public OnRefreshCommand(KategoriBarangKeluarViewModel _vm, IRestApiClientModel _restApi, bool _isTest = false)
        {
            Vm = _vm;
            RestApi = _restApi;
            IsTest = _isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            if (Vm.IsNamaKategoriBarangKeluarChecked)
            {
                Param.Add("Kategori", Vm.FilterNamaKategoriBarangKeluar ?? "");
            }
            Param.Add("currentPage", Vm.CurrentPage);
            Param.Add("pageSize", Vm.LimitData.Key);
            var Response = await Task.Run(() => RestApi.GetAsync($"/api/{ApiVersion}/master-kategori-barang-keluar", Param));
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Data != null)
                {
                    Vm.Data = Result.Data.ToObject<ObservableCollection<MasterKategoriBarangKeluarDto>>();
