...
namespace SiPintar.Commands.Gudang.MasterData.TipeBarang
{
    public class OnRefreshCommand : AsyncCommandBase
    {
        ...
        public OnRefreshCommand(TipeBarangViewModel _vm, IRestApiClientModel _restApi, bool _isTest = false)
        {
            Vm = _vm;
            RestApi = _restApi;
            IsTest = _isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            if (Vm.IsNamaTipeBarangChecked)
            {
                Param.Add("NamaTipeBarang", Vm.FilterNama ?? "");
            }
            Param.Add("currentPage", Vm.CurrentPage);
            Param.Add("pageSize", Vm.LimitData.Key);
            var Response = await Task.Run(() => RestApi.GetAsync($"/api/{ApiVersion}/master-tipe-barang", Param));
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Data != null)
                {
                    Vm.Data = Result.Data.ToObject<ObservableCollection<MasterTipeBarangDto>>();
