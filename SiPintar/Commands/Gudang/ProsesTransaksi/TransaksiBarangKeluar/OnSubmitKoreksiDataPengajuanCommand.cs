...
{
    public class OnSubmitKoreksiDataPengajuanCommand : AsyncCommandBase
    {
        private readonly TransaksiBarangKeluarViewModel _vm;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnSubmitKoreksiDataPengajuanCommand(TransaksiBarangKeluarViewModel vm, IRestApiClientModel restApi, bool isTest = false)
        {
            _vm = vm;
            _restApi = restApi;
            _isTest = isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var response = await Task.Run(() =>
                _restApi.PatchAsync(
                    $"/api/{_restApi.GetApiVersion()}/supervisi-pengajuan-pengeluaran-barang-koreksi", null, param));
