...
{
    public class OnGenerateNoRegistrasiCommand : AsyncCommandBase
    {
        private readonly TransaksiBarangKeluarViewModel _vm;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnGenerateNoRegistrasiCommand(TransaksiBarangKeluarViewModel vm, IRestApiClientModel restApi, bool isTest = false)
        {
            _vm = vm;
            _restApi = restApi;
            _isTest = isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var response = await Task.Run(() =>
                _restApi.GetAsync(
                    $"/api/{_restApi.GetApiVersion()}/pengajuan-pengeluaran-barang-generate-nomor-registrasi", param));
            if (!response.IsError)
            {
                if (response.Data.Status)
                {
                    ...
                    if (res.Count > 0)
                    {
                        ...
                        _vm.Form.NomorRegistrasiWpf = obj.GetValue("NomorRegistrasi", StringComparison.OrdinalIgnoreCase).ToString();
                    }
                }
                else
                {
