...
{
    public class OnSubmitSpkCommand : AsyncCommandBase
    {
        ...
        public OnSubmitSpkCommand(SupervisiPengajuanPembelianViewModel _vm, IRestApiClientModel _restApi, bool _isTest = false)
        {
            Vm = _vm;
            RestApi = _restApi;
            IsTest = _isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            if (Vm.Spk != null)
            {
                ...
                PropertyInfo[] prop = typeof(ParamPengajuanPembelianBarangSuratPerjanjianDto).GetProperties();
                foreach (PropertyInfo item in prop)
                {
                    if (item.Name != "IdPdam" && item.Name != "IdUserRequest")
                    {
                        Param.Add(item.Name, item.GetValue(Vm.Spk));
                    }
                }
                var Response = await Task.Run(() => RestApi.PatchAsync($"/api/{ApiVersion}/supervisi-pengajuan-pembelian-barang-surat-perjanjian", null, Param));
