...
{
    public class OnKoreksiBarangCommand : AsyncCommandBase
    {
        private readonly DaftarBarangMasukViewModel _vm;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnKoreksiBarangCommand(DaftarBarangMasukViewModel vm, IRestApiClientModel restApi, bool isTest = false)
        {
            _vm = vm;
            _restApi = restApi;
            _isTest = isTest;
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            if (param != null)
            {
                ...
                _ = await DialogHelper.ShowDialogGlobalYesCancelViewAsync(_isTest, true, "GudangRootDialog", "Konfirmasi Koreksi Barang",
                    "Koreksi barang pada Nomor Transaksi " + _vm.SelectedDaftarBarangMasuk.NomorTransaksi + " ?", "confirmation",
                    null, moduleName: "gudang", highlightText: _vm.SelectedDaftarBarangMasuk.NomorTransaksi, isBackgroundProcessOnConfirm: true, yesAction: () =>
                    {
                        _ = Task.Run(async () =>
                        {
                            ...
                            var res = await _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/barang-masuk-koreksi-barang", null, param);
