...
{
    public class OnOpenLihatFotoRumahCommand : AsyncCommandBase
    {
        private readonly SupervisiViewModel _viewModel;
        private readonly bool _isTest;
        private string DialogIdentifier;

        public OnOpenLihatFotoRumahCommand(SupervisiViewModel viewModel, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var UrlFoto = Path.Combine(Bulan + Tahun, $"{_viewModel.SelectedData.NoSamb}R.jpg");
            if (parameter != null)
                UrlFoto = parameter as string;

            await ((AsyncCommandBase)_viewModel.GetFotoRumahCommand).ExecuteAsync(UrlFoto);
        ...
        {
            _ = DialogHost.Show(new FotoRumahView(_viewModel), DialogIdentifier);
        }
