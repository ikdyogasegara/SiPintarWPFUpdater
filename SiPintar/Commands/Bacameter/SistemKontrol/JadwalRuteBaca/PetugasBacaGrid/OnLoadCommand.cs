...
namespace SiPintar.Commands.Bacameter.SistemKontrol.JadwalRuteBaca.PetugasBacaGrid
{
    public class OnLoadCommand : AsyncCommandBase
    {
        ...
        public OnLoadCommand(PetugasBacaGridViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var Response = await _restApi.GetAsync($"/api/{ApiVersion}/summary-rute-baca-per-petugas-baca", param);
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status && Result.Data != null)
                {
                    var data = Result.Data.ToObject<ObservableCollection<SummaryPetugasBacaDto>>();
