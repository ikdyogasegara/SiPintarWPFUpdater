...
namespace SiPintar.Commands.Bacameter.SistemKontrol.DaftarKelainan
{
    public class OnLoadCommand : AsyncCommandBase
    {
        ...
        public OnLoadCommand(DaftarKelainanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var Response = await _restApi.GetAsync($"/api/{ApiVersion}/master-kelainan", param);
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status)
                {
                    var data = Result.Data.ToObject<ObservableCollection<MasterKelainanDto>>();
