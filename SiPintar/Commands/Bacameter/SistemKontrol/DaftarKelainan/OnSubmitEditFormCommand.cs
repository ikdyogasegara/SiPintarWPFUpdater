...
namespace SiPintar.Commands.Bacameter.SistemKontrol.DaftarKelainan
{
    public class OnSubmitEditFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditFormCommand(DaftarKelainanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdKelainan", _viewModel.FormData.IdKelainan }
            };
            ...
            {
                if (property.Name != "IdPdam" && property.Name != "IdUserRequest" && property.Name != "IdKelainan")
                {
            ...
            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-kelainan", null, body));
            if (!Response.IsError)
            {
