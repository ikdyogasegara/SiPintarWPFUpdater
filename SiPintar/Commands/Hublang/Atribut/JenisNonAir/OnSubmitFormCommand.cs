...
namespace SiPintar.Commands.Hublang.Atribut.JenisNonAir
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(JenisNonAirViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            _viewModel.FormData.DetailBiaya = _viewModel.FormDataDetail.ToList();

            Type type = typeof(MasterJenisNonAirDto);
            ...
            if (_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/master-jenis-non-air", null, body));
            else
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-jenis-non-air", body));
