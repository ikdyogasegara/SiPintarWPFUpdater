...
namespace SiPintar.Commands.Hublang.Atribut.TipePendaftaran
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(TipePendaftaranViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            Type type = typeof(MasterTipePendaftaranSambunganDto);
            ...
            if (_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/master-tipe-pendaftaran-sambungan", null, body));
            else
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-tipe-pendaftaran-sambungan", body));
