...
namespace SiPintar.Commands.Hublang.Atribut.Peruntukan
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(PeruntukanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            Type type = typeof(MasterPeruntukanDto);
            ...
            if (_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/master-peruntukan", null, body));
            else
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-peruntukan", body));
