...
{
    public class OnSubmitUsulanFormCommand : AsyncCommandBase
    {
        private readonly PermohonanViewModel _viewModel;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnSubmitUsulanFormCommand(PermohonanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        private async Task ShowCetakDialogAsync(string nomorPermohonan)
        {
            if (!_isTest)
            {
                ...
                {
                    case "Pelanggan Air":
                        ((OnCetakCommand)_viewModel.OnCetakCommandPelangganAirUsulan).IsCetak = true;
                        ((OnCetakCommand)_viewModel.OnCetakCommandPelangganAirUsulan).IsPreview = true;
                        ((OnCetakCommand)_viewModel.OnCetakCommandPelangganAirUsulan).TemplateName = $"{fitur}PelangganAir_{_viewModel.SelectedTipePermohonanComboBox.KodeTipePermohonan}";
                        ((OnCetakCommand)_viewModel.OnCetakCommandPelangganAirUsulan).CustomParam = param;
                        ...
                        await DialogHelper.ShowDialogGlobalYesCancelViewAsync(
                            ...
                            "success",
                            _viewModel.OnCetakCommandPelangganAirUsulan,
                            "Cetak",
                            "Tutup",
                            moduleName: "distribusi");
