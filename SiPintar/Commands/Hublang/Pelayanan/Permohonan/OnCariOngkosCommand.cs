...
namespace SiPintar.Commands.Hublang.Pelayanan.Permohonan
{
    ...
    public class OnCariOngkosCommand : AsyncCommandBase
    {
        private readonly PermohonanViewModel _viewModel;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnCariOngkosCommand(PermohonanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            RestApiResponse Response = await _restApi.GetAsync($"/api/{_restApi.GetApiVersion()}/master-ongkos", body);
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status && Result.Data != null)
                {
                    _viewModel.DaftarOngkos = Result.Data.ToObject<ObservableCollection<MasterOngkosDto>>();
            ...
            _viewModel.IsEmptyFormOngkos = _viewModel.TotalRecordMaterialOngkos == 0;
            _viewModel.IsLoadingCariMaterialOngkos = false;
