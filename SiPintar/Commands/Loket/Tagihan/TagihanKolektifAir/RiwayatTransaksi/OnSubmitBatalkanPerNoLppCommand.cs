...
{
    public class OnSubmitBatalkanPerNoLppCommand : AsyncCommandBase
    {
        private readonly RiwayatTransaksiViewModel _viewModel;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnSubmitBatalkanPerNoLppCommand(RiwayatTransaksiViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/payment-batal-per-nomor-transaksi", null, param));
            if (!response.IsError)
            {
