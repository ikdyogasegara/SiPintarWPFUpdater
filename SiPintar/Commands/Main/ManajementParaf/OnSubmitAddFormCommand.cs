...
namespace SiPintar.Commands.Main.ManajementParaf
{
    public class OnSubmitAddFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitAddFormCommand(ManajementParafViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-paraf", body));
            if (!response.IsError)
            {
