...
{
    public class OnSubmitAddUserCommand : AsyncCommandBase
    {
        private readonly DaftarPenggunaViewModel _viewModel;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnSubmitAddUserCommand(DaftarPenggunaViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "Aktif", _viewModel.FormData.Aktif },
                { "NoIdentitas", _viewModel.FormData.NoIdentitas },
                { "IdRole", _viewModel.FormData.IdRole },
                { "IdLoket", _viewModel.FormData.IdLoket }
            };

            if (_isTest)
                body.Add("TestId", _viewModel.TestId);

            var Response = await Task.Run(() => _restApi.PostAsync($"/api/{ApiVersion}/master-user", body));
            if (!Response.IsError)
            {
