...
namespace SiPintar.Commands.Akuntansi.MasterData.MasterDataAkuntansi.Interaksi.InteraksiPenyusutan
{
    public class OnSubmitDeleteFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitDeleteFormCommand(InteraksiPenyusutanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var param = new Dictionary<string, dynamic>
            {
                ...
                { "IdInPenyusutan", _viewModel.SelectedData.IdInPenyusutan! }
            };

            var response = await Task.Run(() => _restApi.DeleteAsync($"/api/{_restApi.GetApiVersion()}/master-inpenyusutan", param));
            if (!response.IsError)
            {
