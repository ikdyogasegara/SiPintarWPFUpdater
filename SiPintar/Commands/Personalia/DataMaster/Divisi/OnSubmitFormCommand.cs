...
namespace SiPintar.Commands.Personalia.DataMaster.Divisi
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(DivisiViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                {"Divisi", _viewModel.FormDivisi },
                {"IdDivisiAtas", _viewModel.FormDivisiAtas },
                {"IdDepartemen", _viewModel.FormDepartemen },
                {"Urutan", _viewModel.FormUrutan },
            };

            RestApiResponse Response = null;
            if (!_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-divisi", body));
            else
            {
                body.Add("IdDivisi", _viewModel.SelectedData.IdDivisi);
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/master-divisi", null, body));
            }
