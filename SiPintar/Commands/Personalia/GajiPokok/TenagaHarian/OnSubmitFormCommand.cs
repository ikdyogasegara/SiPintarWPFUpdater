...
namespace SiPintar.Commands.Personalia.GajiPokok.TenagaHarian
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(TenagaHarianViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                {"Tipe", _viewModel.FormKeteranganGaji },
            };

            RestApiResponse Response = null;
            if (!_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-gaji-pegawai-harian", body));
            else
            {
                body.Add("IdGajiHarian", _viewModel.SelectedData.IdGajiHarian);
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/master-gaji-pegawai-harian", null, body));
            }
