...
namespace SiPintar.Commands.Personalia.GajiPokok.TenagaKontrak
{
    public class OnLoadCommand : AsyncCommandBase
    {
        ...
        public OnLoadCommand(TenagaKontrakViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var Response = await Task.Run(() => _restApi.GetAsync($"/api/{_restApi.GetApiVersion()}/master-gaji-pegawai-kontrak", param));
            _viewModel.TenagaKontrakList = new ObservableCollection<MasterPegawaiGajiKontrakDto>();
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status && Result.Data != null)
                {
                    _viewModel.TenagaKontrakList = Result.Data.ToObject<ObservableCollection<MasterPegawaiGajiKontrakDto>>();
                }
                else
            ...
            _viewModel.IsEmpty = !_viewModel.TenagaKontrakList.Any();
