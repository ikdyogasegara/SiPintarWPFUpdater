...
namespace SiPintar.Commands.Personalia.Tunjangan.MasterTunjangan
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(MasterTunjanganViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                {"KodeJenisTunjangan", _viewModel.FormKode },
                {"NamaJenisTunjangan", _viewModel.FormNamaTunjangan },
                {"Urutan", _viewModel.FormUrutan },
                {"Tipe", _viewModel.FormTipe },
            };

            RestApiResponse Response = null;
            if (!_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/master-jenis-tunjangan", body));
            else
            {
                body.Add("IdJenisTunjangan", _viewModel.SelectedData.IdJenisTunjangan);
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/master-jenis-tunjangan", null, body));
            }
