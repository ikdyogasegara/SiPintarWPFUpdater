...
namespace SiPintar.Commands.Personalia.Kepegawaian.Pensiun
{
    public class OnLoadCommand : AsyncCommandBase
    {
        ...
        public OnLoadCommand(PensiunViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var Response = await Task.Run(() => _restApi.GetAsync($"/api/{_restApi.GetApiVersion()}/master-pegawai-jadwal-pensiun", param));
            _viewModel.PensiunList = new ObservableCollection<MasterPegawaiDto>();
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status && Result.Data != null)
                {
                    _viewModel.PensiunList = Result.Data.ToObject<ObservableCollection<MasterPegawaiDto>>();
                }
                else
            ...
            _viewModel.IsEmpty = !_viewModel.PensiunList.Any();

            await GetAgamaAsync();
            await GetAreaKerjaAsync();
            await GetDepartemenAsync();
            await GetDivisiAsync();
            await GetGolonganPegawaiAsync();
        ...
        public async Task GetGolonganPegawaiAsync()
        {
            _viewModel.IsLoading = true;
