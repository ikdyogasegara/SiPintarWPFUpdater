...
namespace SiPintar.Commands.Personalia.Kepegawaian.SKCalonPegawai
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(SKCalonPegawaiViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            _viewModel.FormDetailList.ForEach(f => f.NoPegawai = f.NoPegawaiBaru);
            ...
            if (!_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/mutasi-status-capeg", body));
            else
            {
                body["NoSk"] = _viewModel.FormSk;
                body.Add("IdMutasi", _viewModel.SelectedData.IdMutasi);
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/mutasi-status-capeg", null, body));
            }
