...
namespace SiPintar.Commands.Personalia.Kepegawaian.SKCalonPegawai
{
    public class OnLoadCommand : AsyncCommandBase
    {
        ...
        public OnLoadCommand(SKCalonPegawaiViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var Response = await Task.Run(() => _restApi.GetAsync($"/api/{_restApi.GetApiVersion()}/mutasi-status-capeg", param));
            _viewModel.SKCalonPegawaiList = new ObservableCollection<MutasiStatusCapegDto>();
            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status && Result.Data != null)
                {
                    _viewModel.SKCalonPegawaiList = Result.Data.ToObject<ObservableCollection<MutasiStatusCapegDto>>();
                }
                else
            ...
            _viewModel.IsEmpty = !_viewModel.SKCalonPegawaiList.Any();
