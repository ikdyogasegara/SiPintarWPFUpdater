...
namespace SiPintar.Commands.Personalia.Kepegawaian.SKPegawaiTetap
{
    public class OnSubmitFormCommand : AsyncCommandBase
    {
        ...
        public OnSubmitFormCommand(SKPegawaiTetapViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            if (!_viewModel.IsEdit)
                Response = await Task.Run(() => _restApi.PostAsync($"/api/{_restApi.GetApiVersion()}/mutasi-status-pegawai-tetap", body));
            else
            {
                body["NoSk"] = _viewModel.FormSk;
                body.Add("IdMutasi", _viewModel.SelectedData.IdMutasi);
                Response = await Task.Run(() => _restApi.PatchAsync($"/api/{_restApi.GetApiVersion()}/mutasi-status-pegawai-tetap", null, body));
            }
