...
namespace SiPintar.Commands.Billing.Atribut.Meteran.MerkMeter
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(MerkMeterViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "KodeMerekMeter", _viewModel.KodeForm },
                { "NamaMerekMeter", _viewModel.NamaForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-merek-meter", null, body));
            if (!Response.IsError)
            {
