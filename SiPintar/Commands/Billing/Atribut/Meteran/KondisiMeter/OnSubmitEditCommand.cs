...
namespace SiPintar.Commands.Billing.Atribut.Meteran.KondisiMeter
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(KondisiMeterViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdKondisiMeter", _viewModel.SelectedData.IdKondisiMeter },
                { "KodeKondisiMeter", _viewModel.KodeForm },
                { "NamaKondisiMeter", _viewModel.NamaForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-kondisi-meter", null, body));
            if (!Response.IsError)
            {
