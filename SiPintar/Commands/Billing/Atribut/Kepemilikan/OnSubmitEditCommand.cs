...
namespace SiPintar.Commands.Billing.Atribut.Kepemilikan
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(KepemilikanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdKepemilikan", _viewModel.SelectedData.IdKepemilikan },
                { "KodeKepemilikan", _viewModel.KodeKepemilikanForm },
                { "NamaKepemilikan", _viewModel.NamaKepemilikanForm },
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-kepemilikan", null, body));
            if (!Response.IsError)
            {
