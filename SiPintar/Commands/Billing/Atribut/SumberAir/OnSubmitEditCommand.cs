...
namespace SiPintar.Commands.Billing.Atribut.SumberAir
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(SumberAirViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdSumberAir", _viewModel.SelectedData.IdSumberAir },
                { "KodeSumberAir", _viewModel.KodeForm },
                { "NamaSumberAir", _viewModel.NamaForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-sumber-air", null, body));
            if (!Response.IsError)
            {
