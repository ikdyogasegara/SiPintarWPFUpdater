...
namespace SiPintar.Commands.Billing.Atribut.Status
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(StatusViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "NamaStatus", _viewModel.NamaForm },
                { "Rekening_Air_Include", _viewModel.IncludeRekeningAirForm },
                { "Rekening_Limbah_Include", _viewModel.IncludeRekeningLimbahForm },
                { "Rekening_LLTT_Include", _viewModel.IncludeRekeningLLTTForm },
                { "TanpaBiayaPemakaianAir", _viewModel.TanpaBiayaAirForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-status", null, body));
            if (!Response.IsError)
            {
