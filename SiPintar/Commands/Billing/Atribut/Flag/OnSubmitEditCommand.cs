...
namespace SiPintar.Commands.Billing.Atribut.Flag
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(FlagViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "NamaFlag", _viewModel.NamaForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-flag", null, body));
            if (!Response.IsError)
            {
