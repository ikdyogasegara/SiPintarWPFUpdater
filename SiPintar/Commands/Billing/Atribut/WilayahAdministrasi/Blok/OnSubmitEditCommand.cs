...
namespace SiPintar.Commands.Billing.Atribut.WilayahAdministrasi.Blok
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(BlokViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdBlok", _viewModel.SelectedData.IdBlok },
                { "KodeBlok", _viewModel.KodeBlokForm },
                { "NamaBlok", _viewModel.NamaBlokForm },
                { "IdRayon", _viewModel.RayonForm.IdRayon }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-blok", null, body));
            if (!Response.IsError)
            {
