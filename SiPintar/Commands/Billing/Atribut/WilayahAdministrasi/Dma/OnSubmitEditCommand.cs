...
namespace SiPintar.Commands.Billing.Atribut.WilayahAdministrasi.Dma
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(DmaViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdDma", _viewModel.SelectedData.IdDma },
                { "KodeDma", _viewModel.KodeForm },
                { "NamaDma", _viewModel.NamaForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-dma", null, body));
            if (!Response.IsError)
            {
