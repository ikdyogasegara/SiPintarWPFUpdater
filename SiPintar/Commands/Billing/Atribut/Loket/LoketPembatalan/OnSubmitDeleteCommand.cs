...
namespace SiPintar.Commands.Billing.Atribut.Loket.LoketPembatalan
{
    public class OnSubmitDeleteCommand : AsyncCommandBase
    {
        ...
        public OnSubmitDeleteCommand(LoketPembatalanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdAlasanBatal", _viewModel.SelectedData.IdAlasanBatal }
            };

            var Response = await Task.Run(() => _restApi.DeleteAsync($"/api/{ApiVersion}/master-alasan-batal", entityId));
            if (!Response.IsError)
            {
