...
namespace SiPintar.Commands.Billing.Atribut.Kolektif
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(KolektifViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "KodeKolektif", _viewModel.KodeKolektifForm },
                { "NamaKolektif", _viewModel.NamaKolektifForm },
                { "Keterangan", _viewModel.KeteranganForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-kolektif", null, body));
            if (!Response.IsError)
            {
