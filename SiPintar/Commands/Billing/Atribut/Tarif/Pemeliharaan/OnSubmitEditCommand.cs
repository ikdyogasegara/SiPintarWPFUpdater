...
namespace SiPintar.Commands.Billing.Atribut.Tarif.Pemeliharaan
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(PemeliharaanViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "KodePemeliharaanLain", _viewModel.KodePemeliharaanForm },
                { "NamaPemeliharaanLain", _viewModel.NamaPemeliharaanForm },
                { "Pemeliharaan", _viewModel.BiayaPemeliharaanForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-tarif-pemeliharaan-lain", null, body));
            if (!Response.IsError)
            {
