...
namespace SiPintar.Commands.Billing.Atribut.Tarif.GolonganLimbah
{
    public class OnLoadCommand : AsyncCommandBase
    {
        ...
        public OnLoadCommand(GolonganLimbahViewModel viewModel, IRestApiClientModel restApi, bool isTest = false, Dictionary<string, dynamic> testBody = null)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var Response = await _restApi.GetAsync($"/api/{ApiVersion}/master-tarif-limbah", _testBody);

            _viewModel.MasterGolonganList = new ObservableCollection<MasterTarifLimbahDto>();

            if (!Response.IsError)
            {
                var Result = Response.Data;
                if (Result.Status && Result.Data != null)
                    _viewModel.MasterGolonganList = Result.Data.ToObject<ObservableCollection<MasterTarifLimbahDto>>();
                else
