...
namespace SiPintar.Commands.Billing.Atribut.Tarif.GolonganLimbah
{
    public class OnSubmitDeleteCommand : AsyncCommandBase
    {
        ...
        public OnSubmitDeleteCommand(GolonganLimbahViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdTarifLimbah", _viewModel.SelectedData.IdTarifLimbah }
            };

            var Response = await Task.Run(() => _restApi.DeleteAsync($"/api/{ApiVersion}/master-tarif-limbah", entityId));
            if (!Response.IsError)
            {
