...
namespace SiPintar.Commands.Billing.Atribut.Tarif.Materai
{
    public class OnSubmitDeleteCommand : AsyncCommandBase
    {
        ...
        public OnSubmitDeleteCommand(MateraiViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "KodePeriodeMulaiBerlaku", _viewModel.SelectedData.KodePeriodeMulaiBerlaku }
            };

            var Response = await Task.Run(() => _restApi.DeleteAsync($"/api/{ApiVersion}/master-meterai", entityId));
            if (!Response.IsError)
            {
