...
namespace SiPintar.Commands.Billing.Atribut.Tarif.AdministrasiLain
{
    public class OnSubmitEditCommand : AsyncCommandBase
    {
        ...
        public OnSubmitEditCommand(AdministrasiLainViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "IdAdministrasiLain", _viewModel.SelectedData.IdAdministrasiLain},
                { "KodeAdministrasiLain", _viewModel.KodeAdministrasiForm },
                { "NamaAdministrasiLain", _viewModel.NamaAdministrasiForm },
                { "Administrasi", _viewModel.BiayaAdministrasiForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-tarif-administrasi-lain", null, body));
            if (!Response.IsError)
            {
