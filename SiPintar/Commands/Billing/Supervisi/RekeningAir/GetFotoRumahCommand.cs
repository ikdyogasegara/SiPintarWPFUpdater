...
{
    public class GetFotoRumahCommand : AsyncCommandBase
    {
        private readonly RekeningAirViewModel _viewModel;
        private readonly bool _isTest;
        private int _section = 1; // 1: Thumbnail Foto Rumah, 2: Thumbnail Foto Rumah Baca Ulang, 3: File Master Foto Rumah

        private readonly string _folderPath = Path.Combine(Path.GetTempPath(), "PDAM-SiPintar-Net");

        public GetFotoRumahCommand(RekeningAirViewModel viewModel, bool isTest = false)
        {
        ...
        private async Task GetImageAsync(string urlFile)
        {
            ...
            if (_section == 1)
            {
                _viewModel.ThumbnailFotoRumah = result;
                _viewModel.HasFotoRumah = hasFoto;
            }
            else if (_section == 2)
            {
                _viewModel.ThumbnailFotoRumahUlang = result;
                _viewModel.HasFotoRumahUlang = hasFoto;
            }
            else
                _viewModel.FileFotoRumah = result;
        }
    }
}
