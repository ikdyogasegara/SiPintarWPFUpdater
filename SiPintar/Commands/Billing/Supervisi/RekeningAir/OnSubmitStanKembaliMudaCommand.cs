...
{
    public class OnSubmitStanKembaliMudaCommand : AsyncCommandBase
    {
        private readonly RekeningAirViewModel _viewModel;
        private readonly IRestApiClientModel _restApi;
        private readonly bool _isTest;

        public OnSubmitStanKembaliMudaCommand(RekeningAirViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            if (!response.IsError)
            {
                ...
                if (result.Status)
                {
                    selectedData.TaksasiWpf = body["Taksasi"];
