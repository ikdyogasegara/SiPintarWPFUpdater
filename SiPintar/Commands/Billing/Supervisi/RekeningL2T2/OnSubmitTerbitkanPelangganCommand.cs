...
{
    public class OnSubmitTerbitkanPelangganCommand : AsyncCommandBase
    {
        ...
        public OnSubmitTerbitkanPelangganCommand(RekeningL2T2ViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        {
            if (_viewModel.TerbitkanPelangganList == null)
                return;
            ...
            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/rekening-lltt-terbitkan-pelanggan", null, body));
            if (!Response.IsError)
            {
