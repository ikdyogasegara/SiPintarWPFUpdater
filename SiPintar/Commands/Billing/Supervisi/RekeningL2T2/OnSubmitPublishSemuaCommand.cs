...
namespace SiPintar.Commands.Billing.Supervisi.RekeningL2T2
{
    public class OnSubmitPublishSemuaCommand : AsyncCommandBase
    {
        ...
        public OnSubmitPublishSemuaCommand(RekeningL2T2ViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                PelangganList[i] = (int)item.IdPelangganLltt;
                i++;
            }
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "IdPelangganLltt", PelangganList },
                { "PasswordUser", _viewModel.ConfirmationPasswordForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/rekening-lltt-publish", null, body));
            if (!Response.IsError)
            {
        ...
        private void LoadingDialog()
        {
            ...
            _ = DialogHost.Show(new GlobalLoadingDialog(null, "Mempublish Semua Rek. L2T2", $"{_viewModel.PeriodeFilter.NamaPeriode}", false, false, EstimationTimeInSecond), "BillingRootDialog");
        }
