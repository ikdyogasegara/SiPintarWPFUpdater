...
namespace SiPintar.Commands.Billing.Supervisi.RekeningL2T2
{
    public class OnSubmitKoreksiRekeningCommand : AsyncCommandBase
    {
        ...
        public OnSubmitKoreksiRekeningCommand(RekeningL2T2ViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var body = new Dictionary<string, dynamic>
            {
                ...
                { "IdPelangganLltt", (int)_viewModel.SelectedData.IdPelangganLltt },
                { "Biaya", _viewModel.BiayaKoreksiForm }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/rekening-lltt-update", null, body));
            if (!Response.IsError)
            {
