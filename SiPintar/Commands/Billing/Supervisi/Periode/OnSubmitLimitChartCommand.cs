...
{
    public class OnSubmitLimitChartCommand : AsyncCommandBase
    {
        ...
        public OnSubmitLimitChartCommand(PeriodeViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            {
                { "Value", _viewModel.LimitChart }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/master-periode-rekening-chart-limit", null, entityId));
            if (!Response.IsError)
            {
