...
namespace SiPintar.Commands.Billing.Supervisi.PelangganLimbah
{
    public class OnSubmitPerbaruiDataRekeningCommand : AsyncCommandBase
    {
        ...
        public OnSubmitPerbaruiDataRekeningCommand(PelangganLimbahViewModel viewModel, IRestApiClientModel restApi, bool isTest = false)
        {
        ...
        public override async Task ExecuteAsync(object parameter)
        {
            ...
            var ListIdPelangganLimbah = new int[] { (int)_viewModel.SelectedData.IdPelangganLimbah };

            var body = new Dictionary<string, dynamic>
            {
                { "IdPeriode", _viewModel.PeriodePerbaruiForm.IdPeriode },
                { "IdPelangganLimbah", ListIdPelangganLimbah }
            };

            var Response = await Task.Run(() => _restApi.PatchAsync($"/api/{ApiVersion}/rekening-limbah-perbarui-data", null, body));
            if (!Response.IsError)
            {
        ...
        {
            _ = DialogHost.Show(new GlobalLoadingDialog(null, "Rekening Limbah", "sedang diperbarui"), "BillingRootDialog");
        }
