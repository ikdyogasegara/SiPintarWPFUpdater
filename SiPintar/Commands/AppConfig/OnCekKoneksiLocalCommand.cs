...
namespace SiPintar.Commands.AppConfig
{
    ...
    public class OnCekKoneksiLocalCommand : AsyncCommandBase
    {
        private readonly AppConfigViewModel _viewModel;
        private readonly bool _isTest;
        private readonly bool _isReport;

        public OnCekKoneksiLocalCommand(AppConfigViewModel viewModel, bool isReport = false, bool isTest = false)
        {
        ...
        {
            if (string.IsNullOrEmpty(_isReport ? _viewModel.ApiLocalReportUrl : _viewModel.ApiLocalUrl))
                return;
            ...
            try
            {
                var Url = _isReport ? _viewModel.ApiLocalReportUrl : _viewModel.ApiLocalUrl;
                if (Url[^1] == '/')
                    Url = Url[0..^1];
        ...
        private void ShowSuccess()
        {
            if (!_isTest)
                _ = DialogHost.Show(new DialogGlobalView(
                    $"Koneksi Local Api {(_isReport ? "Report " : "")}Berhasil",
        ...
        private void ShowError()
        {
            if (!_isTest)
                _ = DialogHost.Show(new DialogGlobalView(
                    $"Koneksi Local Api {(_isReport ? "Report " : "")}Gagal",
