...
namespace SiPintar.Views.Distribusi.Atribut.KelainanGantiMeter.GantiMeterRutin
{
    /// <summary>
    ...
    {
        private readonly GantiMeterRutinViewModel Vm;

        public DialogFormView(object dataContext)
        {
            InitializeComponent();
            DataContext = dataContext;
            Vm = dataContext as GantiMeterRutinViewModel;
            RadioDenganBiaya.IsChecked = Vm.IsCheckedDenganBiaya;

            CheckButton();

            if (!Vm.IsAdd)
                TitleDialog.Text = $"Koreksi Jenis Kelainan Ganti Meter - Rutin {Vm.SelectedData?.JenisGantiMeter}";
            else
                TitleDialog.Text = $"Tambah Jenis Kelainan Ganti Meter - Rutin";
        }
