...
using SiPintar.Commands.Bacameter.Produktivitas.Petugas;
using SiPintar.Utilities;

namespace SiPintar.ViewModels.Bacameter.Produktivitas
{
    [ExcludeFromCodeCoverage]
    public class PetugasViewModel : ViewModelBase
    {
        public PetugasViewModel(ProduktivitasViewModel parent, IRestApiClientModel restApi)
        {
            Parent = parent;
        ...
        private ObservableCollection<SummaryProduktifitasBacameterPetugasDto> _produktivitasPetugas { get; set; }
        public ObservableCollection<SummaryProduktifitasBacameterPetugasDto> ProduktivitasPetugas
        {
            get { return _produktivitasPetugas; }
            set
            {
                _produktivitasPetugas = value;
                OnPropertyChanged();
            }
        }
        ...
        {
            Labels = ProduktivitasPetugas.Select(x => x.PetugasBaca).ToArray();

            JumlahBacaSeries = ProduktivitasPetugas.Select(x => (int)x.Baca).AsChartValues();
            JumlahTaksirSeries = ProduktivitasPetugas.Select(x => (int)x.Taksir).AsChartValues();
            JumlahKelainanSeries = ProduktivitasPetugas.Select(x => (int)x.Kelainan).AsChartValues();
        }
    }
}
